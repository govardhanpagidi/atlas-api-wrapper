...
// APIClient manages communication with the MMC Atlas API Helper API
...
type APIClient struct {
	...
	CloudBackupScheduleAPI CloudBackupScheduleAPI

	CloudBackupSnapshotAPI CloudBackupSnapshotAPI

	ClusterAPI ClusterAPI

	ClusterRestoreAPI ClusterRestoreAPI

	CollectionAPI CollectionAPI

	DatabaseAPI DatabaseAPI

	DatabaseUserAPI DatabaseUserAPI
}
...
func NewAPIClient(cfg *Configuration) *APIClient {
	...
	// API Services
	c.CloudBackupScheduleAPI = (*CloudBackupScheduleAPIService)(&c.common)
	c.CloudBackupSnapshotAPI = (*CloudBackupSnapshotAPIService)(&c.common)
	c.ClusterAPI = (*ClusterAPIService)(&c.common)
	c.ClusterRestoreAPI = (*ClusterRestoreAPIService)(&c.common)
...
func NewMMCClient(url string) *APIClient {
	userAgent := "atlas-api-wrapper/0.0.1"
...
func (e GenericOpenAPIError) Model() AtlasResponse {
	return e.model
}

// SetModel sets model instance: Should be only used for testing
func (e *GenericOpenAPIError) SetModel(errorModel AtlasResponse) {
	e.model = errorModel
}
...
func formatErrorMessage(status, path, method string, v AtlasResponse) string {
	return fmt.Sprintf("%v %v: HTTP %v (Error code: %q) Detail: %v Reason: %v. Params: %v",
		method, path, status, v.GetErrorCode(),
		v.GetErrorMessage(), v.Status, v.GetResponse())
}
